...
Cross-checked every feature claim on the public marketing surfaces against `src/lib/gates.ts`, the `(public)` route group, and the AI/draft pipeline. **No feature gate currently lies** — paid-only surfaces are correctly blocked at runtime. The risk is in marketing copy that promises Founding Crew benefits we haven't built yet, plus two pricing/copy mismatches called out below.

### Crew Plus — 6 of 6 public claims shipped ✅

Source: [PricingReveal.tsx](src/components/marketing/PricingReveal.tsx) Crew Plus tier + [FeatureShowcase.tsx](src/components/marketing/FeatureShowcase.tsx) tiles 1–5 + [SubscriptionPanel.tsx](src/components/account/SubscriptionPanel.tsx).
...
### Founding Crew — 1 of 7 public claims shipped 🔨

Source: [PricingReveal.tsx](src/components/marketing/PricingReveal.tsx) Founding Crew tier + [FAQ.tsx](src/components/marketing/FAQ.tsx) Q5.
...
> Note: `is_founder` on `profiles` is the **platform-admin** flag (gates `/admin/applications/*`), not the Founding Crew tier. Don't conflate them.
...
- 📋 **Promised** — committed to via Founding Crew or landing-page copy; build deadline matters
- 🔮 **Future** — known direction, no commitment yet

---

## Crew Plus (the working tier — £9/mo · £79/yr)
...
| Crew Plus Stripe checkout | ✅ | `createCheckoutSession` action; webhook validated |

---

## Founding Crew (the concierge tier — £179/yr lifetime · capped 500)
...
| Stripe webhook updates Founding Crew count | 📋 | Auto-decrement remaining slots |
...
| Three-tier pricing reveal | 📋 | Free / Crew Plus / Founding Crew; live counter |
...
| PMF survey at month 1 of paid | 📋 | "How would you feel if you couldn't use Tripcrew?" — Vohra's gold-standard metric |
...
|---|---|---|
| **Polaroid memory stack** (currently shipped on trip overview hero) | Doesn't match the rest of the UI per founder review. Lower transformational signal than bookings/ledger/feed. **Removed from Crew Plus marketing benefits as of 2026-04-27.** | Keep as-is in app, rework visually, or replace with a different memory artifact (e.g. the auto-generated memory book promised to Founding Crew)? |
...
| Bookings agent (actual booking, not just suggestions) | Big undertaking, MCP/API integrations | Founding Crew v2 |
| Multi-trip dashboard (manage 5+ active trips) | Useful for power users, not core promise | Founding Crew v2 |
...
- **When you ship a Founding Crew feature:** flip 📋 → ✅, add the commit hash + date.
...
- **The Founding Crew section is the audit trail of what you owe paying customers.** Treat it more strictly than the others.
